...
ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper left', prop={'size': 20, 'weight': 'bold', 'family': font_name}, frameon=False)
...
ax1.set_ylim(2, 13)
ax1.set_yticks(np.arange(2, 13, 2))
...
lines_1, labels_1 = ax1.get_legend_handles_labels()
lines_2, labels_2 = ax1_secondary.get_legend_handles_labels()
ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper left', prop={'size': 20, 'weight': 'bold', 'family': font_name}, bbox_to_anchor=(0.01, 0.7), frameon=False)
...
def run_reg(df, predictors, target):
    ...
    equation_terms = " + ".join([f"{coefficients[i]:.2f}*{predictor}" for i, predictor in enumerate(predictors)])
    model_equation = f"{target} = {equation_terms} + {intercept:.2f}"
    
    # Print model summary
    print(f"Model equation: {model_equation}")
    print(f'R-squared: {r2:.2f}, RMSE: {rmse:.2f}, MAE: {mae:.2f}, AIC: {aic:.2f}')
...
def run_reg(df, predictors, target):
    ...
    # Extract R-squared, AIC, and Durbin-Watson directly from summary
    r2 = model.rsquared
    aic = model.aic

    # Extract residuals for RMSE and MAE calculation
    residuals = y_test - y_pred
...
def run_reg_with_interaction(df, predictors, interaction_terms, target):
    ...
    intercept = reg.intercept_
    coefficients = reg.coef_
    equation_terms = " + ".join([f"{coefficients[i]:.4f}*{predictor}" for i, predictor in enumerate(all_predictors)])
    model_equation = f"{target} = {equation_terms} + {intercept:.4f}"
    
    print(f"Model equation: {model_equation}")
    print(f'Number of training samples: {len(X_train)}, Number of testing samples: {len(X_test)}')
    print(f'R-squared: {r2:.2f}, RMSE: {rmse:.2f}, MAE: {mae:.2f}')
...
# # Best model with variale anomaly
...
EHI_monthly_stats = calculate_statistics(EHI_df, group_by_field='Month', target_field='HOBO_EHI', 
                                predictor_field='PA_EHI', location_id_field='Location ID')
...
EHI_hourly_stats = calculate_statistics(EHI_df, group_by_field='Hour', target_field='HOBO_EHI', 
                                predictor_field='PA_EHI', location_id_field='Location ID')
...
# Set labels
ax1.set_xlabel('Month', labelpad=12, fontsize=20, fontweight='bold', fontname=font_name)
ax1.set_ylabel('Error (°F)', labelpad=15, fontsize=20, fontweight='bold', fontname=font_name)
ax1.set_ylim(5, 18)
ax1.set_yticks(np.arange(6, 18, 2))
...
monthly_data = [EHI_df[EHI_df['Month'] == m]['HOBO_EHI'] for m in range(1, 13)] 
...
ax2.set_xlabel('Month', labelpad=12, fontsize=20, fontweight='bold', fontname=font_name)
ax2.set_ylabel('HOBO Extended Heat Index (°F)', labelpad=15, fontsize=20, fontweight='bold', fontname=font_name)
ax2.tick_params(axis='both', labelsize=20, width=2)
plt.setp(ax2.get_yticklabels(), fontweight='bold', fontname=font_name)
visible_months = np.arange(1, 13, 2)
ax2.set_xticks(visible_months)
...
ax1.set_ylabel('Error (°F)', labelpad=15, fontsize=20, fontweight='bold', fontname=font_name)
ax1.set_ylim(5, 16)
ax1.set_yticks(np.arange(6, 17, 3))
...
ax1.set_xticklabels([f'{h}:00' for h in visible_hours], fontsize=20, fontweight='bold', fontname=font_name)
ax1.set_ylim(3, 22)
ax1.set_yticks(np.arange(3, 22, 3))
...
ax1_secondary.set_ylim(0, 1.2)
ax1_secondary.set_yticks(np.arange(0, 1.2, 0.3))
ax1_secondary.tick_params(axis='y', labelsize=16, width=2)
plt.setp(ax1_secondary.get_yticklabels(), fontweight='bold', fontname=font_name)
...
hourly_data = [EHI_df[EHI_df['Hour'] == h]['HOBO_EHI'] for h in range(24)]
